...
class WhiteBoardView 
{
    ...
    public function showWhiteBoard($availableWhiteBoards, $content, $wbid){
        $template = new Template(DIRECTOR_SITE.SLASH."views".SLASH."TPL".SLASH."whiteboard.tpl");        
    
        $whiteBoards = "";
        for($i=0; $i<count($availableWhiteBoards); $i++){
            $whiteBoards.=("<a href=\"".$_SESSION["siteAddress"]."WhiteBoardController/showWhiteBoard/".$availableWhiteBoards[$i]["whiteboardID"]."\">".$availableWhiteBoards[$i]["topic"]."</a><br>");
        }
        ...
        $template->assign('WhiteBoardID', $wbid);   
